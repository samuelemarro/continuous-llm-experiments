...
def run_experiment(model, tokenizer, sentence, interest_threshold, save_path, xlabel, legend):
    ...
    interpolation_factors = np.range(20) #np.linspace(0, 10, 100, endpoint=True)
    ...
    for interpolation_factor in interpolation_factors:
        ...
        out = model(input_ids=tokens.unsqueeze(0), position_ids=position_ids, return_dict=True)
        #out = get_outputs_from_continuous_inputs(model, input_ids=tokens.unsqueeze(0), position_ids=position_ids, integration_start=interpolation_factor, return_dict=True)
