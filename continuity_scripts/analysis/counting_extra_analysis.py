...
with plt.style.context(style_info):
    for pure in [False, True]:
        ...
        for model_name in model_configs:
            ...
            print(f'{model_name_short} {np.mean(peak_rates):.4} {np.mean(counterfactual_peak_rates):.4} {np.mean(improvements):.4}')

        print(f'Global {np.mean(all_peak_rates):.4} {np.mean(all_counterfactual_peak_rates):.4} {np.mean(all_improvements):.4}')
