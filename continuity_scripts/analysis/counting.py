    ...
    for model_name in model_configs:
        ...
        for threshold in np.linspace(0, 1, 101, endpoint=True):
            ...
            for i, data in enumerate(setups):
                save_path = f'results/batched/counting/{i}/counting-{i}-{model_name_short}'
        ...
        plt.tight_layout()
        path = Path(f'continuity_scripts/analysis/counting/graphs/completeness_purity_{model_name_short}.png')
        pdf_path = Path(f'continuity_scripts/analysis/counting/graphs/completeness_purity_{model_name_short}.pdf')
        path.parent.mkdir(parents=True, exist_ok=True)
        plt.savefig(str(path))
        plt.savefig(str(pdf_path))

        completeness_auc = np.trapezoid(curve_completeness, dx=0.01)
        counterfactual_completeness_auc = np.trapezoid(curve_counterfactual_completeness, dx=0.01)

        #print(model_name, f'{counterfactual_completeness_auc:.4f}', f'{completeness_auc:.4f}')
        ...
        plt.ylabel('IoU')
        plt.tight_layout()
        path = Path(f'continuity_scripts/analysis/counting/graphs/iou/iou_{model_name_short}.png')
        pdf_path = Path(f'continuity_scripts/analysis/counting/graphs/iou/iou_{model_name_short}.pdf')
        path.parent.mkdir(parents=True, exist_ok=True)
        plt.savefig(str(path))
        plt.savefig(str(pdf_path))

        #print('Num skipped:', num_skipped / total)
        print(model_name, f'{(1 - num_skipped / total) * 100:.4}%')
